...
function bb_foundation_pagination($p = 2, $return = false) {
    ...
    if ($paged > 1) {
        $output .= bb_p_link($paged - 1, 'previous');
    }
    if ($paged > $p + 1) {
        $output .= bb_p_link(1, 'first');
    }
    if ($paged > $p + 2) {
    ...
    for($i = $paged - $p; $i <= $paged + $p; $i++) { // Middle pages
        ...
        if ($i > 0 && $i <= $max_page) {
            $i == $paged ? $output .= "<li class='current' {$rel}><a href='#'>{$i}</a></li> " : $output .= bb_p_link($i);
        }
    }
    ...
    if ($paged < $max_page - $p) {
        $output .= bb_p_link($max_page, 'last');
    }
    if ($paged < $max_page) {
        $output .= bb_p_link($paged + 1, 'next');
    }
    if (!empty($output)) {
